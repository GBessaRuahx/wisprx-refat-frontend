...
const UserModal = ({ open, onClose, userId }) => {
	...
	const handleClose = () => {
      ...
      const userData = { ...values, whatsappId, queueIds: selectedQueueIds, allTicket: values.allTicket, super: values.super, owner: values.owner };
		try {
			if (userId) {
			...
			>
				...
				>
                       ...
                       <OnlyForSuperUser
                               ...
                               yes={() => (
                                       <FormControl variant="outlined" margin="dense" className={classes.maxWidth} fullWidth>
                                               <InputLabel id="role-selection-label-modal">{i18n.t("userModal.form.role")}</InputLabel>
                                               <Select
                                                       labelId="role-selection-label-modal"
                                                       id="role-selection-modal"
                                                       value={values.owner ? "owner" : values.super ? "admin" : "none"}
                                                       onChange={(e) => {
                                                               const val = e.target.value;
                                                               setFieldValue("owner", val === "owner");
                                                               setFieldValue("super", val === "owner" || val === "admin");
                                                       }}
                                                       label={i18n.t("userModal.form.role")}
                                               >
                                                       <MenuItem value="none">{i18n.t("userModal.form.roleNone")}</MenuItem>
                                                       <MenuItem value="admin">{i18n.t("userModal.form.roleAdmin")}</MenuItem>
                                                       <MenuItem value="owner">{i18n.t("userModal.form.roleOwner")}</MenuItem>
                                               </Select>
                                       </FormControl>
                               )}
                       />
